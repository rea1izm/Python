...
str_1 = equation.index('=') +1
str_2 = equation.index('x')
k = int(equation[str_1:str_2])
str_3 = equation.index('+')+1
b = float(equation[str_3:])
y = k*x + b
print(y)
...
month2 = [2, 4, 6, 9, 11]
alarm = 'Вы ввели дату неверного формата.'
dd, mm, yy = date.split('.')
if len(mm) == 2 and len(dd) == 2 and len(yy) == 4:
    mm = int(mm)
    dd = int(dd)
    yy = int(yy)
    if 1 <= yy <= 9999:
